...
@tf.function
def train_step(net, optimizer, window, next_close):
    close = window[-1, 3]
    reward = (next_close - close) / close  # 1-step return
    ...
    with tf.GradientTape() as tape:
        ...
        # Actor loss
        logp = tf.math.log(policy[0,1] + 1e-8)
        actor_loss = -logp * tf.stop_gradient(advantage)

        # Critic loss
        critic_loss = tf.square(advantage)

        # Entropy bonus (VERY IMPORTANT)
        entropy = -tf.reduce_sum(
            policy * tf.math.log(policy + 1e-8)
        )
    ...
    for symbol in range(data.shape[0]):
        ...
        for i in range(data.shape[1] - window_size - 1):
            ...
            window = tf.convert_to_tensor(
                data[symbol, i:i+window_size],
                tf.float32
            )
            curr_close = data[symbol, i+window_size-1, 3]
            next_close = data[symbol, i+window_size, 3]

            loss, policy, _ = train_step(net, optimizer, window, next_close)
            epoch_loss += float(loss)

            # true label
            true_label = 1 if next_close > curr_close else 0

            # predicted label
            pred_label = int(tf.argmax(policy, axis=1).numpy()[0])

            # update accuracy
            if pred_label == true_label:
                correct += 1
            total += 1
            position, cash, action, pnl = simulate_trade_step( policy, curr_close, next_close, position, cash ) # Mark-to-market equity 
            equity = cash + (position * next_close) 
            equity_curve.append(equity)
        
            if i % 50 == 0: 
                print(f"Step {i}, action={action}, equity={equity:.2f}")
                print(i/len(data[0]- window_size - 1)*100,'%_curr-accuracy: ',correct/total)
